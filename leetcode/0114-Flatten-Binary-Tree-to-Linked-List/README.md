...
If find, insert all the subtree into the current node's right-side.

![P114_idea_diagram](./P0114_idea.png)
...
public:
    void flatten(TreeNode* root) {
        ...
        TreeNode* rend = root->left;
        while (rend->right) rend = rend->right;  // find the rightmost node for root->left
        rend->right = root->right;
